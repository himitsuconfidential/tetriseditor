...
class Setup:
    ...
    def search(self, subid):
        ...
        if parentsubid not in self.members:
            showerror(title = 'search error', message = f' invalid setup id ! the savefile may be corrupted!')
            return None
        if childid != '':
    ...
    def insert(self, newsetup):
        ...
        if parent is None:
            showerror(title = 'insert error', message = f' invalid setup id [{newsetup.fullid}]! the savefile may be corrupted!')
